...
and generate ImportAssembly & QC configuration files
...
def main():
    ...
    # open import_assembly_pipeline.conf
    pipeline_import_assembly = open('%s/conf/%s/import_assembly_pipeline.conf' % (options.root, options.category), 'w')
    ...
    for line in assembly_lines:
        ...
        if project != previous_project:
            ...
            # create one import assembly conf file specific per project
            import_conf_filename = '%s/conf/%s/%s_import_assembly.conf' % (options.root, options.category, project)
            import_conf = open(import_conf_filename, 'w')
            import_conf.write(constants.IMPORT_CONF_TEMPLATE % {'root':options.root,
                                                                'category':options.category,
                                                                'db':constants.DATABASE[options.category],
                                                                'db_host':os.getenv('VRTRACK_HOST'),
                                                                'db_port':os.getenv('VRTRACK_PORT'),
            ...
            log.info("Import assembly conf file %s has been generated." % import_conf_filename)

            # update import_assembly_pipeline.conf
            pipeline_import_assembly.write("__PathTrack_Import__\t%s\n" % (import_conf_filename))

            # update previous project name
            previous_project = project

    pipeline_qc.close()
    pipeline_import_assembly.close()
